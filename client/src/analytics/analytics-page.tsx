...
export default function AnalyticsPage() {
    ...
    if(data) return <div>
        <div>
            <DatePicker value={{startDate, endDate}} handleChange={handleDateChange}/>
        </div>
        <div>
            <div>
                <div>кількість створених замовлень на день</div>
                <AnalyticsGraph data={convertArray(data.amount)} name="кількість"/>
            </div>
            <div>
                <div>кількість отриманих замовлень за увесь час</div>
                <AnalyticsGraph data={convertArray(data.doneAmount)} name="кількість"/>
            </div>
            <div className="flex justify-center p-6">
                <button type="button" onClick={generatePdf}>Завантажити звіт</button>
            </div>
        </div>
    </div>
    else return <div></div>
